    ...
    , ui(new Ui::PracticeWidget)
{
    ...
    // // //字符匹配功能
    // // //初始化业务状态
    // // //在一开始告诉service有一个label上面有文字
    // // fontservice->setTargetText(ui->label->text());
    // // originalTargetText = ui->label->text();
    // // //连接ui-》service
    // // connect(ui->lineEdit,&QLineEdit::textChanged,fontservice,&FontService::processInput);

    // // //连接service->ui,ui要负责把结果变成颜色
    // // connect(fontservice,&FontService::matchResultReady,this,[=](const QVector<bool> &results){

    // //     ui->label->setTextFormat(Qt::RichText);

    // //     QString target = originalTargetText;
    // //     // 1. 构造富文本（HTML）
    // //     QString richText;

    // //     //遍历目标字符串的每个字符，结合匹配结果设置颜色
    // //     // 2. 根据 results[i] 决定颜色
    // //     for (int i = 0; i < target.length(); ++i) {
    // //         //处理边界情况，如果results的长度小于字符索引，默认视为不匹配
    // //         bool isMatch = (i < results.size()) ? results[i] : false;
    // //         // 设置颜色：匹配为绿色，不匹配为红色
    // //         QString color = isMatch ? "green" : "red";
    // //         // 拼接富文本片段（注意转义特殊字符，比如&、<、>等，避免HTML解析错误）
    // //         QString ch = target.mid(i, 1);
    // //         richText += QString("<span style=\"color:%1;\">%2</span>")
    // //                         .arg(color)
    // //                         .arg(ch.toHtmlEscaped());
    // //     }

    // //     // 3. 设置到 label
    // //     ui->label->setText(richText);
    // // });


    // //动态生成QLabel和QLineEdit方法
    // // 1. 初始化目标文本
    // originalTargetText =
    //     "语言优美：所谓优美，就是指散文的语言清新明丽（也美丽），生动活泼，富于音乐感，行文如涓涓流水，叮咚有声，如娓娓而谈，情真意切。所谓凝练，是说散文的语言简洁质朴，自然流畅，寥寥数语就可以描绘出生动的形象，勾勒出动人的场景，显示出深远的意境。散文力求写景如在眼前，写情沁人心脾。语言优美：所谓优美，就是指散文的语言清新明丽（也美丽），生动活泼，富于音乐感，行文如涓涓流水，叮咚有声，如娓娓而谈，情真意切。所谓凝练，是说散文的语言简洁质朴，自然流畅，寥寥数语就可以描绘出生动的形象，勾勒出动人的场景，显示出深远的意境。散文力求写景如在眼前，写情沁人心脾。语言优美：所谓优美，就是指散文的语言清新明丽（也美丽），生动活泼，富于音乐感，行文如涓涓流水，叮咚有声，如娓娓而谈，情真意切。所谓凝练，是说散文的语言简洁质朴，自然流畅，寥寥数语就可以描绘出生动的形象，勾勒出动人的场景，显示出深远的意境。散文力求写景如在眼前，写情沁人心脾。语言优美：所谓优美，就是指散文的语言清新明丽（也美丽），生动活泼，富于音乐感，行文如涓涓流水，叮咚有声，如娓娓而谈，情真意切。所谓凝练，是说散文的语言简洁质朴，自然流畅，寥寥数语";

    // // 2. 计算 label 可用宽度
    // int labelWidth = ui->scrollArea->width() - 40;
